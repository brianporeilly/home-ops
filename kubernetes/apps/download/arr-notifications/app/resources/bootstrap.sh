...
register_jellyfin() {
  ...
  existing="$(curl -s -H "X-Api-Key: ${api_key}" "${base_url}/api/${api_path}/notification" \
    | jq -r '.[]? | select(.name=="Jellyfin") | .id' | head -n1)"

  payload=$(cat <<JSON
{
  "name": "Jellyfin",
  "onGrab": true, "onDownload": true, "onUpgrade": true, "onRename": true,
  "implementation": "MediaBrowser",
  "implementationName": "Emby/Jellyfin",
  "configContract": "MediaBrowserSettings",
  "fields": [
    {"name": "host", "value": "jellyfin.media.svc.cluster.local"},
...
JSON
  ...
  if [ "$status" -ge 200 ] && [ "$status" -lt 300 ]; then
    echo "${app_name}: Jellyfin notification synced (status $status)"
  else
    echo "${app_name}: failed to sync Jellyfin notification (status $status):"
    cat "$response_file"
    echo
    FAILURES=$((FAILURES + 1))
  fi
...
register_jellyfin "Sonarr" "http://sonarr.download.svc.cluster.local:8989" "v3" "${SONARR_API_KEY}"
register_jellyfin "Radarr" "http://radarr.download.svc.cluster.local:7878" "v3" "${RADARR_API_KEY}"
